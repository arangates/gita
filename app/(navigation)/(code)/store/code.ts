...
const SLOKA_SAMPLES: CodeSample[] = [
  {
...
export const codeExampleAtom = atom<CodeSample | null>(SLOKA_SAMPLES[Math.floor(Math.random() * SLOKA_SAMPLES.length)]);

export const isCodeExampleAtom = atom<boolean>(
  (get) => !!SLOKA_SAMPLES.find((codeSample) => codeSample.code === get(codeAtom)),
);
